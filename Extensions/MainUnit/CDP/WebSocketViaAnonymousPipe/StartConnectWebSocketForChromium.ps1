...
#
#	このスクリプトは、匿名パイプとして繋ぐ構造上、Excelのセルに埋め込んで使用して下さい
# ====================================================================================
...
# ※VBA側での環境変数設定が必要です
# --- 1-1. 接続先の設定 ---
[string]$wsUrl    = $env:fromVBA_wsUrl	#`remote-debugging-pipe`相当の`ws`に接続します。`http://127.0.0.1:9222/json/version`にて、確認可能です。
[uint16]$port     = $env:fromVBA_port	#接続リストを検索する際のデフォルトポート番号

# --- 1-2. VBAから渡されたハンドル番号を元に、FileStreamHandleを作成 ---
$hIn = [System.IO.FileStream]::new([IntPtr]([int64]$env:fromVBA_hCDPInRd), 'Read')
$hOut = [System.IO.FileStream]::new([IntPtr]([int64]$env:fromVBA_hCDPOutWr), 'Write')
#----------------------------------------------------------------------------------------
...
#----------------------------3. パイプ接続と接続待機処理----------------------------
try {
    # サーバー/クライアントという概念ではなく「一方向のストリーム」としてパイプ定義します
    $pipeIn = $hIn	#バイナリデータでやり取りするので直接渡す
    $pipeOut =$hOut	#バイナリデータでやり取りするので直接渡す
} catch {
    Write-Error "💫 匿名パイプへの接続中に、エラーが発生しました。`nコンソールの再起動が必要です`n$($_.Exception.Message)"
    exit 1
}
...
try {
    ...
    $taskReadPipe = $pipeIn.ReadAsync($bufferPipe, 0, $bufferPipe.Length)
    ...
    # どちらかが切断されるまで無限ループ
    while ($pipeIn.CanRead -and $ws.State -eq 'Open') {
    ...
    Log "🛑 通信終了"
    if ($pipeIn -ne $null) { $pipeIn.Dispose() }
    if ($pipeOut -ne $null) { $pipeOut.Dispose() }
