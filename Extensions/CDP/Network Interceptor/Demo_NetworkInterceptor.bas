...
'
' ■ テスト環境
'   ForDevelopers\OperationCheck\CDP\TestHtml\Test_NetworkInterceptor\TestHtml.html
' =========================================================

Private Const TEST_HTML_PATH As String = "file:///[WORKSPACE]\ForDevelopers\OperationCheck\CDP\TestHtml\Test_NetworkInterceptor\TestHtml.html"
...
Public Sub Demo_NetworkInterceptor_All()
    ...
    Set br = 設定シートからのCDP起動(TEST_HTML_PATH)
...
Private Sub Demo_A_BlockURL(br As CDPBrowser, ni As CDPexpansion_NetworkInterceptor)
    br.printMsg info_, "▶ [A] URLブロック テスト開始", "Demo"

    ' httpbin.org/get をブロック登録
    ...
    If InStr(1, res, "blocked", vbTextCompare) > 0 Or InStr(1, res, "Blocked", vbTextCompare) > 0 Then
        br.printMsg info_, "  ✔ URLブロック成功！fetchがエラーになりました。", "Demo"
        MsgBox "[A] URLブロック成功！" & vbCrLf & "結果: " & res, vbInformation, "Demo"
    Else
        br.printMsg WARN_, "  ✘ URLブロックが効いていません。結果: " & res, "Demo"
...
Private Sub Demo_B_MockResponse(br As CDPBrowser, ni As CDPexpansion_NetworkInterceptor)
    br.printMsg info_, "▶ [B] モックレスポンス テスト開始", "Demo"

    ' /api/user への通信を偽レスポンスで差し替える
    Dim mockJson As String
    mockJson = "{""name"":""\u30c6\u30b9\u30c8\u592a\u90ce"",""role"":""admin"",""mock"":true}"
    ...
    If InStr(1, res, "mock", vbTextCompare) > 0 Then
        br.printMsg info_, "  ✔ モックレスポンス成功！", "Demo"
        MsgBox "[B] モックレスポンス成功！" & vbCrLf & "Body: " & res, vbInformation, "Demo"
    Else
        br.printMsg WARN_, "  ✘ モックレスポンスが効いていません。結果: " & res, "Demo"
...
Private Sub Demo_C_WaitForResponse(br As CDPBrowser, ni As CDPexpansion_NetworkInterceptor)
    br.printMsg info_, "▶ [C] WaitForResponse(CDPネイティブ) テスト開始", "Demo"
    ...
    If reqId <> "" Then
        br.printMsg info_, "  ✔ レスポンス検出！ requestId=" & reqId, "Demo"
        ...
        MsgBox "[C] WaitForResponse 成功！" & vbCrLf & _
               ...
               vbInformation, "Demo"
    Else
        br.printMsg WARN_, "  ✘ タイムアウト：レスポンスが検出できませんでした。", "Demo"
