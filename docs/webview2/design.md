...
すべてのCOMコールバックは、機械語で書かれたサンク（後述）を経由します。次の状態は、それぞれ危険度が異なります（v3.2.0でさらに精査）。

- **安全**: `.navigate`等の同期メソッド呼び出しが完了し、普通のVBAコード行でブレーク／ステップ実行が静止している状態。この状態のままブラウザ側からコールバックを後で受け取っても、通常はクラッシュしません
- **危険①**: `.navigate`等、内部で完了まで待つ同期メソッドの**呼び出し中そのもの**でブレークすると、ネイティブ呼び出しの真っ最中にVBEのデバッガが割り込む形になり、クラッシュする可能性があります
- **危険②**: 静止ブレイク中でも、ページ操作やJS遷移等をきっかけに、購読中のCDPイベントや保留中のコマンド完了通知が実際に届くと、機械語サンク経由でVBAへ再入しようとしてクラッシュする可能性があります
- **最も危険**: 上記①②いずれの状態でも、VBEで**リセット操作**を行うのはほぼ確実にクラッシュします（サンク領域やハンドラオブジェクトが解放され、ブラウザ側が保持中の生ポインタが無効化されるため）

どうしてもリセットが必要な場合は、まず処理を安全な静止状態まで進め、イミディエイトウィンドウから`UnsubscribeAllCdpEvents`で購読を解除してから行ってください。
:::

::: warning `UnsubscribeAllCdpEvents`を呼ぶと内部トラッキングも止まる（v3.2.0〜）
`ConnectCDP`成功時、このツールは`Page.frameStartedLoading`等、`WaitEvents`や生存確認に必要なCDPイベントを内部で自動購読しています（[WaitEvents](/guides/navigation)を参照）。デバッグ目的で`UnsubscribeAllCdpEvents`を呼ぶと、これらの内部トラッキングも道連れで止まります。以後`WaitEvents`等が正しく機能しなくなるため、デバッグが終わったら再接続するか、必要なイベントを再購読してください。
...
::: tip クラッシュ調査には外部デバッグログビュアーが便利（v3.2.0〜）
上記のようなクラッシュは、発生した瞬間にイミディエイトウィンドウの内容ごと失われがちです。VBAプロジェクトの条件付きコンパイル引数に `exportDbgMsg = 1` を追加すると、すべてのログが `OutputDebugStringW` 経由でも出力されるようになり、[DebugView](https://learn.microsoft.com/ja-jp/sysinternals/downloads/debugview)（インストール不要）等の外部ビュアーでExcelクラッシュ後も直前のログを確認できます。日常利用では不要な、調査用のオプトイン機能です。
:::
