---
description: v1.0.0 から v2.4.0 まで、61 個のタグに刻まれた 235 日間の記録。Excel VBA 単体で CDP を喋るツールが、寄せ集めのマクロブックからコアエンジンに育つまでのコード差分の物語です。
---
...
この記事は、そこから今日までに起きたことを、**コミットログとコード差分だけを頼りに**振り返る記録です。技術解説ではありません。数字と、そのとき自分が書いたコミットメッセージが語ってくれる、地味で、かなり泥臭い話です。
...
VBA の `String` は UTF-16、CDP が求めるのは UTF-8。ここを `ADODB.Stream` に頼っていると、絵文字（サロゲートペア）でいとも簡単に崩れます。しかも `ADODB.Stream` は COM の外部参照です。

そこで生まれたのが **`CharacterCodeConversion.cls`（新規 221 行）**。`WideCharToMultiByte` / `MultiByteToWideChar` を直接叩いて自前で変換する、専任のクラスです。`WebSocketCommunicator.cls` からは 94 行が消えました。
...
それまで要素は `nodeId`（DOM ツリー上の番号）で扱っていました。これは DOM が書き換わると簡単に無効になります。`objectId`（JavaScript ヒープ上のハンドル）方式に切り替えたことで、`Runtime.callFunctionOn` にそのまま渡せるようになり、要素操作と JavaScript 実行が同じ土俵に乗りました。`CDPElement.cls` は 601 行が書き換わっています。
...
第2章で、文字化けの元凶として追放したはずのクラスです。

理由は単純で、**役割が変わったから**でした。かつては「文字コード変換をやらせて失敗していた」。今回は「バイト列をひたすら溜め込む器」としてだけ使い、変換は自前の `CharacterCodeConversion.cls` が最後に一括で行います。バッファ蓄積の速さにおいて、`ADODB.Stream` は VBA 標準の文字列連結より圧倒的に速い。
...
## 第10章 — WebSocket が「第2の背骨」になる（v2.3.0）

ここまで、ブラウザとの通信路は **匿名パイプ一択**でした。`--remote-debugging-pipe` を使う限り、相手はローカルで自分が起動したブラウザだけです。

7月、`CDPCoreViaWebSocket.cls`（新規 **1,032 行**）が加わります。
...
ポイントは「Pipe 版に準拠した」という部分です。`v2.0.0` で `CDPCore` を通信コアとして切り出しておいたおかげで、**上位の `CDPBrowser` / `CDPContext` を一切変えずに、下の通信路だけ差し替えられる**構造になっていました。第8章の解体が、ここで効いてきます。

続く `v2.3.1` では、接続までを自動化しました。
...
そして、この対応が**当初は想定していなかった扉**を開けます。WebSocket なら相手はローカルである必要がない。結果として、**すでに立ち上がっている目の前のブラウザ**、**Android 実機の Chrome**、**他アプリに埋め込まれた WebView2** まで射程に入りました。
...
flowchart TB
  ...
  g["2026-07 v2.2.0 / v2.3.0<br/>JSON ゼロコピー化<br/>WebSocket が第2の背骨に"]
...
3月に「Json パーサーを回避」と書いた負債は、6月に `BiDiCDPJson.cls` として返済されました。1月に追放した `ADODB.Stream` は、7月に別の役割で戻ってきました。**そのとき妥協で書いたコードは、コミットログの中でずっと待っている。**

::: tip フィナーレ
61 個のタグのうち、胸を張れる大改修は 5、6 個しかありません。残りはタイプミスの修正と、`1 insertion(+), 1 deletion(-)` の恥ずかしいホットフィックスです。

でも、その積み重ねでしか **「安定して動く」** には届かなかった。235 日分のコミットログを読み返して、いちばん強く思ったのはそれでした。
:::
