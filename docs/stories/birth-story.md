---
description: v1.0.0 から v3.0.0 まで、64 個のタグに刻まれた 253 日間の記録。寄せ集めのマクロブックがコアエンジンに育つまでと、WinSock 自力実装で「できなかったこと」ができるようになった夜の話です。
---

# 寄せ集めのマクロブックは、どうやってコアエンジンになったのか

::: tip 誕生秘話
〜v1.0.0 から v3.0.0 まで、64 個のタグに刻まれた 253 日の記録〜
:::
...
いま `git tag` を叩くと、64 個のタグが並びます。先頭は **2025年12月20日 の `v1.0.0`**、末尾は **2026年8月30日 の `v3.0.0`**。その間、253日。
...
`getTab` の `SearchType` が反映されない、というだけのバグ。**1 行のためにタグを切った**わけですが、この「小さくても切る」癖が、後に 64 個という異常なタグ数を生むことになります。
...
::: tip 教訓
同じ 3月22日、リポジトリに `.gitattributes` が追加されています。コミットメッセージは「Gitリポジトリの仕様に対処」。`v1.0.0` の時点には存在せず、`v3.0.0` にはある。**ファイル一覧の差分にも、事故の跡は残る**という話です。
:::
...
## 第12章 — 整理券に、ちゃんと名前をつける（v2.4.1）

`v2.4.0` のわずか2日後、2026年8月16日。1日の作業で **7 ファイル、556 行追加 / 570 行削除**。増えてはいないのに、中身はこの 239 日で一番細かく手が入りました。

### `SendMessage` という名前の終わり

`ExecuteCDPAsync`（第3章・`core-comparison/async.md` にも載っている、あの整理券発行メソッド）は、内部で `Private Function SendMessage` という下請けを呼んでいました。「送って、待つ」を1つの関数に詰め込んだ名残です。

```text
2026-08-16 `ReadyRunCDP`に実行時のcommanIDを返す方式に変更
2026-08-16 `SendMessage`の責務分離化
2026-08-16 `ReadyRunBiDi`も実行時のcommanIDを返す方式に変更
2026-08-16 `SendMessage`の責務分離化
```

きっかけは、通信コアの `ReadyRunCDP`（`Sub` → `Function ... As Long`）が、**実行時の `commandID` をその場で返す**ように変わったことでした。それまでは `Sub` で送りっぱなしにして、直後に `PipeCore.LastCommandID` というプロパティを読みに行く2手順が要りました。ID を返り値としてその場で受け取れるなら、**`LastCommandID` というプロパティ自体が要らない**。そう気づいて削除すると、それを読むためだけに存在していた `SendMessage` の役目も消えました。

> 「送るところと、待つところを、同じ関数の中に閉じ込めておく必要は、もうなかった」

残されたのは、送信だけの `ExecuteCDPAsync` と、結果を待つだけの新顔でした。ここでも命名は一発では決まりません。

```text
2026-08-16 `commandID`の引数名に統一させておく
```

差分を読むと、生まれた直後の名前は `AutoWaitRunCommandID` だったとわかります。それが数時間後、こう改名されました。

```text
2026-08-16 `TakeResultCDP`の自動待機版を用意
```

**`AutoWaitTakeResultCDP`。** `TakeResultCDP`（蓄積済みの結果を覗くだけの素朴なメソッド）の名前を引き継いだうえで、「無ければ待つ」までやる版だと一目でわかるようにした改名です。第8章で `CDPTab` が半日で `CDPContext` に変わったのと、同じ速度の判断でした。BiDi 側にも同じ形で `AutoWaitTakeResultBiDi` が並びます。
...
```text
2026-08-16 引数名修正
2026-08-16 エラー停止用引数名が変わったことによるリネーム
```

`StopPipeError` ―― 通信エラーで止めるかどうかのスイッチです。`v2.3.0`（第10章）で WinSock を自力実装し、パイプと WebSocket が対等な通信路になったあとも、この引数名だけは **「パイプしかなかった頃」の名前のまま**残っていました。`StopApiError` への統一で、ようやく名前が現実に追いつきました。テストコード側の呼び出し（`Test_CDPElement.bas` / `Test_jsEval.bas`）まで、名前付き引数ごと直しています。

### ホバーだけで出現する UI に、手が届くようになった

機能追加は1つだけでした。

```text
2026-08-15 マウスホバーのシミュレート機能を追記
2026-08-16 マウスホバーテストを追加
2026-08-16 ホバーで出現するボタンテストを追加
```

`sendClick`(座標クリック) の隣に、`sendHover` が加わりました。要素の中心座標へ `Input.dispatchMouseEvent`（`mouseMoved`）を送り、物理的なマウス移動をシミュレートします。CSS の `:hover` だけで出現するメニューやツールチップは、JS の合成 `MouseEvent` では反応しないことがあります。座標ベースのクリックがすでにあったので、**同じ道具でホバーも表現できる**という、素直な拡張でした。

### 残りは、全部「地ならし」

```text
2026-08-16 エラーコードの同期
2026-08-16 一部の`Property Get` → `Function` に変更
2026-08-16 要素操作関連の中分類を設定
2026-08-16 見た目を統一
2026-08-16 WebSocket用テストに切り返しやすいように改良
```

WebSocket 経由時のエラーコードをパイプ側と揃え、引数を取らないだけの `Property Get`（`isAvailability` など）を素朴な `Function` に直し、`CDPElement.cls` にセクション見出しのコメントを足す。派手さはゼロですが、どれも **「後で読む自分」のための整理**です。

::: tip この章の位置づけ
新機能は `sendHover` の1つだけ。残りは全部、**名前と責務を現実に合わせ直す作業**でした。`v2.0.0` で God クラスを解体し、`v2.4.0` で `TakeEvents` を3分割し、この `v2.4.1` で `SendMessage` という最後の「兼任メソッド」を手放す。239 日かけて、キットは少しずつ **「その名前が、その役割だけをする」** 状態に近づいています。
:::

---

## 第13章 — Microsoft製`Dictionary`を、卒業する（v2.4.2）

`v2.4.1` から1週間後、2026年8月23日。45 ファイル・**3,075 行追加 / 184 行削除**。ただし、この行数の大半は新顔のクラスが1つ増えただけの結果です。中身は大きく5つに分かれます。

### 住所が変わった：`VBAProject/` から `src/` へ

```text
2026-08-18 フォルダ名を変更したのでこっちも変更
2026-08-18 xlflowの構成に最適化
2026-08-21 CLAUDE向けの説明書を用意
```

`VBAProject/Class/` → `src/classes/`、`VBAProject/Modules/` → `src/modules/`、`VBAProject/Form/` → `src/forms/`。ついでに `Microsoft Access Objects` / `Microsoft Excel Objects` も `src/workbook/` へ合流しました。

理由は、新しく迎え入れた開発ツール **`xlflow`**（VBEとソースツリーを同期し、テストやフォーマット・静的解析まで面倒を見るCLI）です。設定ファイル `xlflow.toml` が新規153行。以降、このリポジトリの「本体」は`src/`ディレクトリで、これまで手作業に頼っていたVBEへのエクスポート/インポートを`xlflow`任せにできるようになりました。

### `Microsoft Scripting Runtime`を、卒業する 🎓

これが今回の主役です。

```text
2026-08-18 Scripting.Dictionary→Dictionary に統一
2026-08-19 試験的にMicrosoft製のDictionaryを卒業
2026-08-22 `Scripting.Dictionary`→`Dictionary`に変更
```

VBAでKeyを基に値を紐づける、といえばこれまで`Microsoft Scripting Runtime`（`Scripting.Dictionary`）が定番でした。しかし有志の検証によると、Microsoft 365環境・特に非信頼マクロ下では「`DictionaryInDictionary`（Dictionaryの中にDictionaryを持つ構造）」が極端に遅くなることが判明しています。2016時代は問題なかったものの、時代が変わってしまったようでした。

> 「本家の非同期イベント蓄積で、まさにこの`DictionaryInDictionary`を使っていた。確かに処理が遅くなっていた」

そこで、`Scripting.Dictionary`を一切使わない、純VBA実装の互換クラス [`VBA-FastDictionary`](https://github.com/cristianbuse/VBA-FastDictionary)（cristianbuse氏）を`Dictionary.cls`として新規2,555行、丸ごと迎え入れました。第9章で登場した`VBA-FastJSON`と、**同じ作者の手によるもの**です。

効果は抜群で、場面によっては本家`Scripting.Dictionary`より速いという結果も出ました。メソッド互換性もそのままなので、`New Dictionary`と書く限り、既存コードの書き味は一切変わりません。`ExecuteCDP`/`ExecuteCDPAsync`の引数型が`Scripting.Dictionary`から`Dictionary`に変わったのも、この移行の一環です。

::: info もう一つの「借り物からの卒業」
第2章で`ADODB.Stream`を、第9章で旧JSONパーサーを、そして今回`Scripting.Dictionary`を。235日を超えても、**「借りてきた汎用部品を、専用の自作品に置き換える」という同じパターン**が繰り返されています。
:::

### 通信コアの帳尻合わせ

```text
2026-08-21 メモリアロケーションを考慮した拡張ロジックに刷新
2026-08-21 ログにWebSocketフレーム情報を出力するようにする
```

1つ目のコミットメッセージには「WebSocket側もそれに合わせる」という一言が添えられており、Pipe側だけでなくWinSock側のソケットバッファも同時に手が入っています。第11章までは「足りなくなったら倍々に拡張する」という素朴な方針でしたが、倍々を続けると**VBA文字列の上限（`2^30-1`文字）を平気で飛び越えてしまう**ケースが理論上ありえました。最後の1回だけは倍にせず、上限ピッタリに着地するよう調整しています。あわせて、初期バッファサイズも`CreatePipe`が実際に確保するメモリの粒度（アロケーション粒度）に揃え、WinSock側のソケットバッファも同じ数値に合わせました。パイプとソケット、2つの通信路の帳尻を、1つのコミットで同時に合わせにいった格好です。

### `CDPElement`の隠れたバグ

```text
2026-08-22 検索結果情報を`Type`にまとめる
2026-08-22 相対検索時、親情報の`CDPElement.cls`を残すようにする
2026-08-22 相対検索時の`onExist`が機能しない問題を修正
```

要素から相対検索（`el.getElementByQuery(...)`のように、既存の`CDPElement`を起点にした検索）をして、その結果に対して`onExist`（存在するまで待つポーリング）を呼ぶと、**常にトップレベルからの検索がやり直される**という隠れたバグがありました。相対検索である以上、本来は「親要素を起点に」再検索しなければならないのに、その親情報を保持していなかったのが原因です。`CDPElement`が自分の親（`CDPElement`そのもの）を覚えておくように直したことで、相対検索＋`onExist`の組み合わせが正しく動くようになりました。

### テストしやすい開発環境へ

```text
2026-08-23 単体テストをしやすくするための専用テストメソッドを設ける
```

[コアロジック徹底比較](/core-comparison/gaps)で触れている通り、このキットには要素操作や`jsEval`のテストはあっても、通信の心臓部`CDPCore.cls`向けのユニットテストはありませんでした。今回、バッファ拡張ロジック（`StrBufferCheck`）などに対して専用のテスト関数を追加し、条件付きコンパイル引数を立てない限り呼び出せない「隠しテスト」として仕込んでいます。`xlflow`を採用した動機の一つも、まさに**このテストコードを実行しやすくするため**でした。

::: tip この章の位置づけ
新機能はゼロで、卒業と帳尻合わせだけの回でした。`v2.0.0`でGodクラスを解体し、`v2.2.0`で旧JSONパーサーを、`v2.4.1`で`SendMessage`を、そして`v2.4.2`で`Scripting.Dictionary`を手放す。 **「借りてきたものを少しずつ自分の手で作り直す」** という、この物語がずっと繰り返してきた動きの、現時点での最新形です。
:::

---

## 終章 — 半年前に埋めた種を掘り起こす（v3.0.0）

`v2.4.2` から7日後、2026年8月30日。**メジャーバージョンを上げた**、この記録で初めてのタグです。差分は43ファイル、**18,104 行追加 / 1,786 行削除**。163個のコミットが、たった1つのタグに束ねられています。
...
2026-03-15 COM コールバックの中から別の COM 非同期呼び出しをしていた再入問題を修正
2026-03-15 QueryInterface が E_NOINTERFACE を返していた問題を修正
2026-03-15 BeginControllerCreation と同じ VARIANT 構造体ベースの引数渡しが適用できていないのが原因でクラッシュする要因を修正
2026-03-15 `oleaut32.dll`でのクラッシュ問題を修正
2026-03-15 vtable オフセットが間違っている影響でクラッシュする問題を修正
```

**3月15日。** 第7章「機械語を書いた春」で `WinHttpSetStatusCallback` と格闘していたのと、ほぼ同じ時期です。裏では、もう1つの低レイヤ格闘――WebView2 を COM 経由で直接叩く実験が、静かに始まっていました。QueryInterface が `E_NOINTERFACE` を返す、vtable のオフセットが1つずれているだけでクラッシュする、VARIANT の渡し方を間違えると `oleaut32.dll` の中で落ちる。第7章と同じ手触りの、地雷原を這うようなデバッグが続きます。

3月17日〜21日にかけて、`CDPBrowser` にあるメソッド群を移植し、非同期イベント受信の導線を確保し、CDP 向け Demo コードまで一旦動くところまで持っていきました。そして4月4日、「リファクタリング」というコミットを最後に――**このブランチは、ぴたりと止まります**。
...
```text
2026-08-23 クレジット表記追加
2026-08-23 英語部分も修正
2026-08-24 v2.4.2のWebView2機能付きとして再構成
```

**141日間、`WebView2` ブランチは誰にも触られませんでした。** その間、本編では第8章の God クラス解体、第9章の JSON ゼロコピー化、第10章の WinSock 自力実装、第13章の `Scripting.Dictionary` 卒業まで、地形が何度も作り変えられています。凍らせておいたブランチを掘り起こしたとき、待っていたのは「3月の自分が書いたコードが、8月の景色にそのままでは馴染まない」という現実でした。`v2.4.2のWebView2機能付きとして再構成` という1行に、その作業のすべてが圧縮されています。

::: tip 伏線の、また別の返し方
これまでの伏線回収は「1つのコメント」や「1つの妥協」が、数ヶ月後に返済される話でした。今回は**ブランチ1本まるごと**が141日間そのまま待っていた、という今までで一番大きなスケールの回収です。
:::

### 疑似Edgeの卒業

再開後、最初に手を付けたのは奇妙にも「引き算」でした。

```text
2026-08-26 疑似Edgeを卒業
2026-08-26 キオスクモード引数の廃止
```

第1部で扱った Lv.1「`msedge.exe` を Kiosk モードで UserForm にドッキングする」手法――見た目こそ WebView2 でしたが、中身は `--kiosk` 引数で武装した本物の Edge プロセスでした。3月から温めていた「本物の WebView2（`ICoreWebView2`）を直接叩く」実装がついに実用段階に達したことで、**この「疑似WebView2」は丸ごと不要になり**、`KioskMode` 引数ごと削除されました。「借り物を自分の言葉に置き換える」というこの物語の癖が、今度は「代役を本役に置き換える」という形で現れた格好です。

### `CDPCoreHost` の誕生 — ローカルブラウザ機能の集約

WebView2 を本実装に組み込む過程で、`CDPBrowser` と `CDPCore` に散らばっていた「ローカルPC上のブラウザプロセスに関する機能」――起動、ポリシーチェック、後始末、クラッシュ復元プロンプトの無効化――が、新設の `CDPCoreHost.cls` に集約されました（このクラスは v3.1.0 で `CDPHost.cls` に改名されています。以降はこの章の当時の名前のまま記述します）。

```text
2026-08-28 ポリシーチェックのコアをbas側へ移管
2026-08-28 ブラウザのパス取得もbasへ
2026-08-29 ローカルブラウザ自体の機能群をまとめる
2026-08-29 コマンドライン生成を`CDPCoreHost`に委ねるように改良
2026-08-29 `CDPCoreHost,CDPCore`に委託したメソッド分を除去
```

ヘッダーコメントには、こう明記されています。

> ローカルPC上のブラウザに関する機能群です。Chromium系のみ取り扱います。※WebView2を除く

WebView2 は独自のライフサイクル（`Environment`/`Controller`/`ICoreWebView2` の COM 生成）を持つため、`CDPCoreHost` の管轄外です。**「ローカルプロセスとしてのブラウザ」と「COMオブジェクトとしてのWebView2」を、同じクラスに混ぜない**という線引きが、明確に引かれました。第8章で `CDPBrowser` を「ブラウザ」「タブ」「通信コア」に割ったのと同じ手つきで、今回は「起動」がもう1段独立した格好です。

### WebSocketモードでの新規起動 — `RunWebSocketModeBrowserCDP`

これまで WebSocket モードは「既に起動しているブラウザへの後付け接続」専門でした。`CDPCoreHost` という受け皿ができたことで、Pipe 版の `Start○○ModeContext` と同じ感覚の、起動から接続までを一気に行うメソッドが生まれます。

```text
2026-08-29 WebSocketモードでローカルブラウザを起動するメソッドを新設
2026-08-30 WebSocketによる新規起動で`CDPBrowser`で返却するように改良
```
...
ポリシーチェック・残存セッションの後始末・`DevToolsActivePort` の出現待機・接続までを `CDPCoreHost` に委託し、接続済みの `CDPBrowser` を返す。中身は新顔ですが、やっていることの形は `ExecuteCDPAsync` が整理券を返すのと同じ、 **「面倒な手順を1つの呼び出しに畳み込む」** という、この物語で何度も繰り返されたパターンです。
...
```text
2026-08-27 フォームオブジェクト名を変更
2026-08-30 親しみやすい`This○○`にリネーム
```

WebView2 を埋め込む UserForm は `WebView2Form` という名前に固まり、そこから覗ける現在のタブ・WebView2オブジェクトは `ThisCDPContext` / `ThisWebView2` という、`CDPBrowser.ThisCDPCore` などと同じ「This〜」の命名規則に揃えられました。移植元 [WebView2-For-Excel-VBA](https://github.com/tarboh/WebView2-For-Excel-VBA)（たーぼー氏）の機械語サンク・vtable技法はバイト単位でそのまま活かしつつ、その上に乗るクラス名・プロパティ名は、この235日間で育ててきた自分たちの語彙に置き換える。借り物の技術と、自分たちの言葉づかいを両立させる、これまでと同じやり方でした。

::: tip この章の位置づけ
5ヶ月近く凍っていたブランチが、`Scripting.Dictionary` 卒業や `src/` 移行という**足場の総入れ替え**の直後に解凍され、その新しい足場の上で「疑似Edgeの卒業」「`CDPCoreHost` への集約」「WebSocket起動の一本化」という3つの整理を経てマージされる。メジャーバージョンが上がったのは機能が増えたからというより、**「ブラウザとの接点」という設計そのものが一段引き締まった**からだと感じています。
:::
...
| クラス | v1.0.0 | v1.6.0 | v1.8.1.2 | v2.0.0 | v2.4.0 | v2.4.1 | v2.4.2 | v3.0.0 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `CDPBrowser.cls` | 1,506 | 2,580 | **3,310** | 1,357 | 1,471 | 1,453 | 1,467 | 1,111 |
| `CDPContext.cls` | — | — | — | 2,053 | 2,371 | 2,351 | 2,350 | 2,376 |
| `CDPCore.cls` | 369 | 458 | 502 | 1,076 | 1,345 | 1,344 | 1,421 | 1,498 |
| `CDPCoreViaWebSocket.cls` | — | — | — | — | 1,114 | 1,114 | 1,136 | 1,181 |
| `CDPElement.cls` | 673 | 874 | 972 | 987 | 1,227 | 1,270 | 1,298 | 1,298 |
| `WebDriverBiDiCore.cls` | — | 1,095 | 1,224 | 783 | 810 | 808 | 808 | 793 |
| `WebDriverBiDiMode.cls` | — | — | — | 592 | 841 | 825 | 825 | 821 |
| `BiDiCDPJson.cls` | — | — | — | — | 2,860 | 2,860 | 2,860 | 2,860 |
| `Dictionary.cls` | — | — | — | — | — | — | **2,555**（新規） | 2,555 |
| `CDPCoreHost.cls` | — | — | — | — | — | — | — | **642**（新規） |
| `CDPCoreViaWebView2.cls` | — | — | — | — | — | — | — | **598**（新規） |

`CDPBrowser.cls` の折れ線が、この物語のすべてです。**1,506 行で始まり、3,310 行まで太り、1,357 行に砕かれ、育ち直したと思いきや、`CDPCoreHost` への機能移管でさらに 1,111 行まで削られた。**
...
flowchart TB
  ...
  i["2026-08 v2.4.1<br/>SendMessage を廃止・sendHover 追加<br/>整理券に名前をつけ直す"]
  j["2026-08 v2.4.2<br/>Scripting.Dictionary を卒業<br/>src/ 移行・xlflow 導入"]
  k["2026-08 v3.0.0<br/>WebView2ネイティブ対応<br/>141日凍結ブランチの合流"]
...
**2. 借り物を「自分の言葉」に置き換えるのに、半年以上かかった**

`v1.0.0` の README は謝辞だらけでした。いまも先人への敬意は変わりませんが、`CDPCore` も `CDPContext` も `CDPCoreViaWebSocket` も、そして土台の `Scripting.Dictionary` すらも、この 253 日で自分（と、その道の専門家からの借り物）が組み直したものです。

**3. 伏線は、必ず回収される。ときには、ブランチ1本まるごと**

3月に「Json パーサーを回避」と書いた負債は、6月に `BiDiCDPJson.cls` として返済されました。1月に追放した `ADODB.Stream` は、7月に別の役割で戻ってきました。4月に機械語まで書いて生き延びさせようとした WinHTTP は、7月に「死のルート」と見限り、WinSock へ乗り換えました。8月には、`SendMessage` という名前の下に隠れていた「送ると待つの兼任」まで手放し、`Scripting.Dictionary` そのものからも卒業しました。そして同じ8月の終わりに、**3月から4ヶ月半凍らせていた WebView2 ブランチ**まで掘り起こして本編に合流させました。**そのとき妥協で書いたコード（あるいは、そのとき止めた手）は、コミットログの中でずっと待っている。**

::: tip フィナーレ
64 個のタグのうち、胸を張れる大改修は 5、6 個しかありません。残りはタイプミスの修正と、`1 insertion(+), 1 deletion(-)` の恥ずかしいホットフィックスです。

でも、その積み重ねでしか **「安定して動く」** には届かなかった。IE の魔法の杖を失ってから 253 日。いま手元にあるのは、1枚の xlsm を送るだけで、目の前の Edge も、USB の向こうの Android も、Excel自身に埋め込んだWebView2も、同じ顔で動かせる土管です。

253 日分のコミットログを読み返して、いちばん強く思ったのはそれでした。
:::
