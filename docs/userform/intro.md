...
::: info v3.0.0での進展
執筆当初「Lv.99」は「理論上できるが本キットには未実装、他プロジェクトへのリンク紹介」という位置づけでした。v3.0.0で、この技法（COM/vtableハック）を本キット自身に `CDPCoreViaWebView2.cls` / `WebView2Thunks.bas` として実装し、`CDPContext`/`CDPElement`から通常のCDP制御と同じ感覚で使えるようになりました。また、Lv.1（Edge埋め込み）はこの実装に伴い廃止されています。v3.1.0では拡張機能インストール等にも対応しました。詳細は各ページを参照してください。
:::
