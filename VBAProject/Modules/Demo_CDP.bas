...
'***************************************************************************************************
'* 機能　　：CDP の RemoteObject が取りうる全 type / subtype の組み合わせを網羅的に検証します
'---------------------------------------------------------------------------------------------------
'* 詳細説明：公式ドキュメントにある type / subtype の全候補を実際に jsEval で発生させ、
'            JSresultAnalysis がどう処理するかを確認します。
'
'  【type の全候補】
'    object / function / undefined / string / number / boolean / symbol / bigint
'
'  【object の subtype 全候補】
'    array / null / node / regexp / date / map / set / weakmap / weakset /
'    iterator / generator / error / proxy / promise /
'    typedarray / arraybuffer / dataview / webassemblymemory / wasmvalue / trustedtype
'
'  【各テストの記法】
'    - returnByValue:=True  → value フィールドから値を取得するパス
'    - returnByValue:=False → objectId または description フォールバックのパス
'    - StopError:=False     → JS例外が返っても止まらないように
'
'* 注意事項：・webassemblymemory / wasmvalue / trustedtype は環境依存で取れない場合があります
'            ・ぜひ、ログレベル：DEBUG にして実行してみてね
'
'  ★ type:function についての補足 ★
'    Runtime.evaluate は「式の評価結果」を返すため、
'    - `function myFunc(x) {}` （関数宣言文）→ 文なので戻り値なし → type:undefined
'    - `(function myFunc(x) {})` （関数式）→ 関数オブジェクト自体が値 → type:function
'    括弧で囲むことで「式として評価」させる必要があります。
'***************************************************************************************************
Sub jsEval型網羅検証()
    Dim JsonConv As New WebJsonConverter
    Dim res As Variant
    ...
    Set browser = 設定シートからのCDP起動("https://abehiroshi.la.coocan.jp/")

    Debug.Print "============================================="
    Debug.Print "  RemoteObject type/subtype 全網羅テスト 開始"
    Debug.Print "============================================="
    ...
    With browser
        ...
        ' ■ type: string
        '==========================================================================
        Debug.Print vbCrLf & "=== type: string ==="
        res = .jsEval("'Hello CDP!'")
        Debug.Print "  結果: [" & TypeName(res) & "] " & res & " | 期待:String"


        '==========================================================================
        ' ■ type: number
        '==========================================================================
        Debug.Print vbCrLf & "=== type: number ==="

        ' 通常の数値
        res = .jsEval("123.45")
        Debug.Print "  通常数値:  [" & TypeName(res) & "] " & res & " | 期待:Double"

        ' 特殊値 NaN / Infinity / -Infinity / -0 は UnserializableValue として来る
        ' → value フィールドなし, description あり → フォールバックで description を返す
        res = .jsEval("NaN", StopException:=False)
        Debug.Print "  NaN    :  [" & TypeName(res) & "] "

        res = .jsEval("Infinity", StopException:=False)
        Debug.Print "  Infinity:  [" & TypeName(res) & "] "

        res = .jsEval("-Infinity", StopException:=False)
        Debug.Print "  -Infinity: [" & TypeName(res) & "] "

        res = .jsEval("-0", StopException:=False)
        Debug.Print "  -0      :  [" & TypeName(res) & "] "


        '==========================================================================
        ' ■ type: boolean
        '==========================================================================
        Debug.Print vbCrLf & "=== type: boolean ==="
        res = .jsEval("true")
        Debug.Print "  true : [" & TypeName(res) & "] " & res & " | 期待:Boolean/True"

        res = .jsEval("false")
        Debug.Print "  false: [" & TypeName(res) & "] " & res & " | 期待:Boolean/False"


        '==========================================================================
        ' ■ type: undefined
        '==========================================================================
        Debug.Print vbCrLf & "=== type: undefined ==="
        res = .jsEval("undefined")
        Debug.Print "  結果: [" & TypeName(res) & "] IsEmpty=" & IsEmpty(res) & " | 期待:Empty"


        '==========================================================================
        ' ■ type: symbol
        ' → value フィールドなし。description に "Symbol(tag)" が入る
        ' → Case Else で description フォールバック
        '==========================================================================
        Debug.Print vbCrLf & "=== type: symbol (Case Else フォールバック確認) ==="
        res = .jsEval("Symbol('tag')", StopException:=False, returnByValue:=False)
        res = .jsEval("Symbol('tag')", StopException:=False, returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Symbol(tag)"""


        '==========================================================================
        ' ■ type: bigint
        ' → value フィールドは文字列 "42"。description に "42n" が入る
        ' → Case Else に入る
        '==========================================================================
        Debug.Print vbCrLf & "=== type: bigint (Case Else フォールバック確認) ==="
        res = .jsEval("42n", StopException:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""42""または""42n"""


        '==========================================================================
        ' ■ type: function
        '
        ' ★ 重要: Runtime.evaluate は「式の評価結果」を返す
        '    - function myFunc(x) {}          → 関数宣言「文」→ 戻り値なし → type:undefined になる
        '    - (function myFunc(x) { ... })   → 括弧で囲んで「式」にする → type:function になる
        '==========================================================================
        Debug.Print vbCrLf & "=== type: function ==="

        ' NG例（コメントとして残す）
        '   res = .jsEval("function myFunc(x) { return x * 2; }")
        '   → type:undefined が返ってしまう。関数宣言は「文」のため。

        ' OK例: 括弧で囲んで関数式として評価させる
        Set res = .jsEval("(function myFunc(x) { return x * 2; })", returnByValue:=True)
        Debug.Print "  結果: [" & TypeName(res) & "]  期待:[Function] myFunc..."

        ' アロー関数でも同様
        Set res = .jsEval("(x => x * 2)", returnByValue:=True)
        Debug.Print "  Arrow: [" & TypeName(res) & "] | 期待:[Function] ..."


        '==========================================================================
        ' ■ type: object / subtype: null
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: null ==="
        res = .jsEval("null")
        Debug.Print "  結果: [" & TypeName(res) & "] IsNull=" & IsNull(res) & " | 期待:Null"


        '==========================================================================
        ' ■ type: object / subtype: array
        ' → returnByValue:=True でないと value フィールドに中身が入らない
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: array ==="

        ' returnByValue:=True → value(Dictionary) が返る
        Set res = .jsEval("['neko', 'inu', 'dog']", returnByValue:=True)
        If IsObject(res) Then
            Debug.Print "  [returnByValue=True]  Count=" & res.Count & " | 期待:Dictionary Count=3"
        End If

        ' returnByValue:=False → description が返る
        res = .jsEval("[10, 20, 30]", returnByValue:=False)
        Debug.Print "  [returnByValue=False] " & CStr(res) & " | 期待:""Array(3)"""


        '==========================================================================
        ' ■ type: object / subtype: node
        ' → returnByValue:=False で className + description が返る
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: node ==="
        res = .jsEval("document.body", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:HTMLBodyElement (class='body...')"


        '==========================================================================
        ' ■ type: object / subtype: regexp
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: regexp (Case Else) ==="
        res = .jsEval("/^hello.*/gi", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""/^hello.*/gi"""


        '==========================================================================
        ' ■ type: object / subtype: date
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: date (Case Else) ==="
        res = .jsEval("new Date('2026-01-01').toISOString()", returnByValue:=True)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:ISO8601日付文字列"


        '==========================================================================
        ' ■ type: object / subtype: map
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: map (Case Else) ==="
        res = .jsEval("new Map([['key1', 1], ['key2', 2]])", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Map(2)"""


        '==========================================================================
        ' ■ type: object / subtype: set
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: set (Case Else) ==="
        res = .jsEval("new Set([1, 2, 3])", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Set(3)"""


        '==========================================================================
        ' ■ type: object / subtype: weakmap
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: weakmap (Case Else) ==="
        res = .jsEval("new WeakMap()", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""WeakMap"""


        '==========================================================================
        ' ■ type: object / subtype: weakset
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: weakset (Case Else) ==="
        res = .jsEval("new WeakSet()", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""WeakSet"""


        '==========================================================================
        ' ■ type: object / subtype: iterator
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: iterator (Case Else) ==="
        res = .jsEval("[1,2,3].values()", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Array Iterator"""


        '==========================================================================
        ' ■ type: object / subtype: generator
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: generator (Case Else) ==="
        res = .jsEval("(function*() { yield 1; yield 2; })()", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Generator"""


        '==========================================================================
        ' ■ type: object / subtype: error
        '
        ' ★ 重要: jsEval は exceptionDetails があれば先にキャッチして CVErr を返す
        '    throw すると exceptionDetails に入ってしまうため、subtype:error を得るには
        '    try{}catch(e){ return e } で error オブジェクトを「返す」必要がある
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: error ==="
        res = .jsEval("(function(){ try { null.x } catch(e) { return e } })()", returnByValue:=False, StopException:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] IsError=" & IsError(res) & " | 期待:CVErr(xlErrValue)"


        '==========================================================================
        ' ■ type: object / subtype: promise
        ' → 未解決の Promise は returnByValue 不可。description のフォールバック
        '   ※ awaitPromise:=True にすると解決後の中身になるので False で subtype を確認
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: promise (Case Else) ==="
        res = .jsEval("new Promise(r => setTimeout(r, 60000))", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Promise"""


        '==========================================================================
        ' ■ type: object / subtype: proxy
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: proxy (Case Else) ==="
        res = .jsEval("new Proxy({}, {})", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Proxy"""


        '==========================================================================
        ' ■ type: object / subtype: typedarray
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: typedarray (Case Else) ==="
        res = .jsEval("new Int32Array([1, 2, 3, 4])", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Int32Array(4)"""


        '==========================================================================
        ' ■ type: object / subtype: arraybuffer
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: arraybuffer (Case Else) ==="
        res = .jsEval("new ArrayBuffer(16)", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""ArrayBuffer(16)"""


        '==========================================================================
        ' ■ type: object / subtype: dataview
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: dataview (Case Else) ==="
        res = .jsEval("new DataView(new ArrayBuffer(8))", returnByValue:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""DataView(8)"""


        '==========================================================================
        ' ■ type: object / subtype: webassemblymemory
        ' → WebAssembly が利用可能なページのみ。不可の場合 StopError:=False で保護
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: webassemblymemory (Case Else) ==="
        res = .jsEval("new WebAssembly.Memory({ initial: 1 })", returnByValue:=False, StopException:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:""Memory"""


        '==========================================================================
        ' ■ type: object / subtype: trustedtype
        ' → Trusted Types API 対応ページのみ（CSPヘッダーが必要）。通常ページではエラーになる
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: trustedtype (Case Else) ==="
        res = .jsEval("typeof trustedTypes !== 'undefined' ? trustedTypes.createPolicy('test', { createHTML: s => s }) : '(trustedTypes not supported)'", returnByValue:=False, StopException:=False)
        Debug.Print "  結果: [" & TypeName(res) & "] " & CStr(res) & " | 期待:TrustedHTMLPolicy(description) または サポート外文字列"


        '==========================================================================
        ' ■ type: object / subtype: wasmvalue
        ' → WebAssembly GC 拡張の reference 型（実験的仕様）
        ' → 通常ページでは生成不可に近いため、WebAssembly APIの存在確認のみ行う
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: wasmvalue (参考確認のみ) ==="
        res = .jsEval("typeof WebAssembly !== 'undefined' ? 'WebAssembly available' : 'not available'", returnByValue:=False, StopException:=False)
        Debug.Print "  wasmvalue は実験的仕様のため直接生成不可。API存在確認: " & CStr(res)


        '==========================================================================
        ' ■ type: object / subtype: (none) ← 通常のプレーンオブジェクト
        '==========================================================================
        Debug.Print vbCrLf & "=== type:object / subtype: (none) ==="

        Set res = .jsEval("({ x: 1, y: 2, label: 'test' })", returnByValue:=True)
        If IsObject(res) Then
            Debug.Print "  [returnByValue=True]  x=" & res("x") & " y=" & res("y") & " | 期待:Dictionary"
        End If

        res = .jsEval("({ x: 1, y: 2 })", returnByValue:=False)
        Debug.Print "  [returnByValue=False] " & CStr(res) & " | 期待:""Object"""
    ...
    Debug.Print vbCrLf & "============================================="
    Debug.Print "  type/subtype 全網羅テスト 完了！！"
    Debug.Print "============================================="
End Sub
