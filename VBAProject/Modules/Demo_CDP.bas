...
Public Function 設定シートからのCDP起動ForTab(Optional StartURL As String, Optional SwitchUser As String, Optional KioskMode As edgeKioskType) As CDPContext
    ...
    With ShSetting01_StartBrowser
        ...
        Dim ブラウザ名 As String: ブラウザ名 = IIf(.Range(.UseRangeName(4, "Demo_CDP.設定シートからのCDP起動ForTab")).value, "chrome", "edge")
...
Sub iframeShadowRootTest()
    ...
    With captchaDemo
        ...
        Set CloudflareTurnstile = .getTab(Url:="https://challenges.cloudflare.com/cdn-cgi/challenge-platform/", SearchTypeID:=iFrame, doRetrySecond:=5000)    '※見つかるまで、5秒間内部でループされます
