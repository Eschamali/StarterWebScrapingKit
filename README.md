...
4. Added isExist method for CDPElement class to check if an element is found. Unlike onExist which will wait until the element is found which can only be useful for certain scenarios, isExist will just do the check and allow the devs freedom to handle the information returned.
...
3. Enhanced .getIFrame to detect if the targetted iFrame is hosted in another domain which will then require indirect automation on the frame.
